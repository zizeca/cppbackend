...
using Point2I = Point2<int>;  //for compatibility
using Point2d = Point2<double>;
using Point2D = Point2<double>; // for compatibility
...
struct Rect {
  ...
  Rect(Point2<T> pos, Point2<T> size)
      : left(pos.x),
        top(pos.y),
        width(size.x),
        height(size.y) {}
  ...
  constexpr bool contains(const Point2<T>& point) const {
