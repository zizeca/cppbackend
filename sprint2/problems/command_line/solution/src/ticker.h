...
class Ticker : public std::enable_shared_from_this<Ticker> {
 public:
  ...
  Ticker(Strand strand, std::chrono::milliseconds period, Handler handler) : m_strand(strand), m_period(period), m_handler{handler} {}

  void Start() {
    m_last_tick = std::chrono::steady_clock::now();
    /* Выполнить SchedulTick внутри strand_ */
    m_timer.expires_at(m_period);
 ...
 private:
  ...
  Strand m_strand;
  boost::asio::steady_timer m_timer{m_strand};
  std::chrono::steady_clock::time_point m_period;
  Handler m_handler;
